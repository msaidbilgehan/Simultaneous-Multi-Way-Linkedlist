...
node_x_connection.set_Coordinate(x=35, y=2, z=0)
for i, node in enumerate(nodes_x):
    node.set_Data(f"nodes_x:{i}")
    node.set_Coordinate(x=40, y=i*4, z=0)
...
node_y_connection.set_Coordinate(x=25, y=2, z=0)
for i, node in enumerate(nodes_y):
    node.set_Data(f"nodes_y:{i}")
    node.set_Coordinate(x=30, y=i*3, z=0)
...
node_z_connection.set_Coordinate(x=15, y=2, z=0)
for i, node in enumerate(nodes_z):
    node.set_Data(f"nodes_z:{i}")
    node.set_Coordinate(x=20, y=i*2, z=0)
...
node_t_connection.set_Coordinate(x=5, y=2, z=0)
for i, node in enumerate(nodes_t):
    node.set_Data(f"nodes_t:{i}")
    node.set_Coordinate(x=10, y=i*1, z=0)
...
output_Gate.set_Coordinate(x=-20, y=-20, z=0)
